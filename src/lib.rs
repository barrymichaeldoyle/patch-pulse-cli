        ...
        for (name, specified_version_str) in deps {
            match fetch_latest_version(&name).await {
                Ok(latest_version_str) => {
                    let name_version = format!("{}: {}", name, specified_version_str);

                    let latest_version = Version::parse(&latest_version_str).unwrap();
                    let up_to_date_msg = if let Ok(specified_version) = Version::parse(
                        ...
                        match (
                            ...
                            (Ordering::Less, _, _) => "new major update available".bright_yellow(),
                            (_, Ordering::Less, _) => "new minor update available".bright_magenta(),
                            (_, _, Ordering::Less) => "new patch update available".bright_cyan(),
                            _ => "up to date with latest".bright_green(),
                        }
                    } else {
                        "is checked against latest version".normal()
                    };

                    let padded_up_to_date_msg =
                        format!("{:width$}", up_to_date_msg, width = update_msg_fixed_width);
